    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Only shipped orders are shown. Match items with the order and confirm receiving.</p>
        </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {Array.isArray(selectedOrder.items) && selectedOrder.items.map((item) => (
                        <tr key={item.itemCode}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">{item.orderQty}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="mb-6">
                ...
                <textarea value={receivingForm.notes} onChange={(e) => setReceivingForm({...receivingForm, notes: e.target.value})} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Optional notes..." />
              </div>

              <div className="flex justify-end space-x-3">
                <button onClick={() => setSelectedOrder(null)} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">Cancel</button>
                <button onClick={handleSubmitReceiving} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Confirm Receiving</button>
              </div>
            </div>
          </div>
        )}
